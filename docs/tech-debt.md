...
| 2026-07-20 | Duplicated _call_claude retry helper in entities.py (copied from pipeline.py to avoid circular import) | Low | Extract shared Claude API retry helper into backend/processing/utils.py during pipeline integration (Task 9) |
| 2026-07-20 | get_recent_processed_articles returns snake_case but dedup functions expect camelCase — conversion deferred to runner wiring | Low | Add _keys_to_camel bridge in pipeline integration (Task 9) |
